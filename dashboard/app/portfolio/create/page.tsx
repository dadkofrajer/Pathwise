...
import { addActivity, type Activity } from "@/lib/api";
...
export default function CreatePortfolioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.lens || !formData.type || !formData.role_level) {
      alert("Please fill in all required fields (Title, Category, Type, and Role Level)");
      return;
    }
    ...
    // Prepare activity data, ensuring dates are in correct format
    const activityData: Activity = {
      id: `activity_${Date.now()}`,
      title: formData.title.trim(),
      lens: formData.lens as any,
      type: formData.type as any,
      area_of_activity: formData.area_of_activity?.trim() || undefined,
    ...
    Object.keys(activityData).forEach(key => {
      if (activityData[key as keyof Activity] === undefined) {
        delete activityData[key as keyof Activity];
      }
    });
    ...
    try {
      ...
      // Call API
      const result = await addActivity(studentId, activityData);
      ...
      if (isNetworkError) {
        // For network errors, show brief alert and redirect
        alert(
          `Unable to connect to backend server.\n\n` +
          `The activity may not have been saved. Please check your backend server and try again.\n\n` +
          `Redirecting to profile page...`
        );
      } else {
        // For validation/other errors, show error message
        alert(`Error saving activity: ${errorMessage}\n\nPlease check the form and try again.`);
        setIsSubmitting(false);
        return; // Don't redirect on validation errors
      }
    }
    
    // Redirect to profile page (for network errors or after delay)
    setTimeout(() => {
      window.location.href = "/profile";
    }, 500);
  };
      ...
      <main className="ml-64 overflow-y-auto">
        <div className="p-6 max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <h1 className="text-white text-2xl font-semibold">Add Activity to Portfolio</h1>
            <p className="text-gray-400 mt-1">Fill in the details about your activity or achievement</p>
          </div>
